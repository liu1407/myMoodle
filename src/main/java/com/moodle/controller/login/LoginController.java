package com.moodle.controller.login;
...
import com.moodle.service.login.LoginService;
import com.moodle.util.CommonUtils;
import com.moodle.util.TmStringUtils;
...
//		String userType = request .getParameter("userType");
		...
		if(TmStringUtils.isEmpty(username) ||TmStringUtils.isEmpty(password)){
			logger.info("传入参数为空");
...
//				return "redirect:../index";
				...
				//如果用户类型不匹配
				logger.info("用户类型不匹配");
				request.setAttribute("msg", "用户类型不匹配");
				return "index/error";
				...
				//如果其他错误
				logger.info("其他错误");
				request.setAttribute("msg", "其他错误");
				return "index/error";
			}
		}else{
			//如果不存在，代码邮箱和密码输入有误
			logger.info("调用service错误");
			request.setAttribute("msg", "调用service错误");
			return "index/error";
		}
